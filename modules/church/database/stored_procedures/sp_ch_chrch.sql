...
i_ch_status char(1),
i_ch_creation_date datetime,
i_ch_created_by int,
i_ch_mod_date datetime,
i_ch_mod_by int,
...
if i_operation = 'I' /*Operation for insert */ then
	if ifnull(i_ch_name,'') = '' or ifnull(ch_location,'') = '' or ifnull(c_location_cl_id,'') = ''  then
    set w_return = 1;
    leave lbl_manage;
  end if; /*if ifnull(i_cm_first_name,'') ....*/

  insert into c_member (ch_name,ch_status,ch_creation_date,ch_created_by,ch_location,c_location_cl_id) 
